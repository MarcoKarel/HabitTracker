import { FREQUENCY_DAYS, type Habit, type HabitCompletion, type HabitWithCompletions } from './types';
...
export const isDayIncludedInFrequency = (frequency: number, dayOfWeek: number): boolean => {
...
export const getFrequencyDays = (frequency: number): string[] => {
  const days: string[] = [];
...
export const createFrequencyFromDays = (days: number[]): number => {
  return days.reduce((frequency, day) => frequency | Math.pow(2, day - 1), 0);
};

// Streak calculations
export const calculateStreak = (habit: Habit, completions: HabitCompletion[]): {
...
export const calculateCompletionRate = (habit: Habit, completions: HabitCompletion[]): number => {
  const startDate = parseDate(habit.start_date);
  const today = new Date();
...
export const enrichHabitWithCompletions = (habit: Habit, completions: HabitCompletion[]): HabitWithCompletions => {
...
export const getStreakColor = (streak: number): string => {
  ...
  if (streak < 30) return '#yellow';
  if (streak < 100) return '#green';
  return '#purple';
};

export const shouldShowConfetti = (oldStreak: number, newStreak: number): boolean => {
  const milestones = [7, 30, 100, 365];
  return milestones.some(milestone => oldStreak < milestone && newStreak >= milestone);
};

// Export utilities
export const exportHabitsToCSV = (habits: HabitWithCompletions[]): string => {
  const headers = [
    'Title',
    'Description',
    'Frequency Days',
...
export const exportHabitsToJSON = (habits: HabitWithCompletions[]): string => {
